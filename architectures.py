...
DENOISERS_ARCHITECTURES = ["cifar_dncnn", "cifar_dncnn_wide", "memnet", "cifar_UNET", # cifar10 denoisers
                            'imagenet_dncnn', 'imagenet_memnet', "imagenet_UNET" # imagenet denoisers
                        ]
...
def get_architecture(arch: str, dataset: str, pytorch_pretrained: bool=False) -> torch.nn.Module:
    ...
    ## Image Denoising Architectures
        ...
        return model
    elif arch == "cifar_UNET":
        model = UNet(3, 3, bilinear=False).cuda()
#         model = DnCNN(image_channels=3, depth=17, n_channels=64).cuda()
        return model
        ...
        model = torch.nn.DataParallel(UNet(3, 3, bilinear=False)).cuda()
        cudnn.benchmark = True
        return model
    else:

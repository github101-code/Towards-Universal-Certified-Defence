...
def plot_certified_accuracy_per_sigma_best_model(outfile: str, title: str, max_radius: float,
    ...
    for it, sigma in enumerate(sigmas):
        methods_sigma = [method for method in methods if '{}'.format(sigma) in method.quantity.data_file_path]
        accuracies_cert_ours, radii = _get_accuracies_at_radii(methods_sigma, 0, max_radius, radius_step)
    ...
    for it, sigma in enumerate(sigmas):
        methods_sigma_base = [method for method in methods_base if '{}'.format(sigma) in method.quantity.data_file_path]
...
# Paper plots
    ...
    cifar_munet = [
            Line(ApproximateAccuracy("certificates/our-cifar10-sigma{:.2f}_resnet110".format(noise)), "$\sigma = {:.2f}$".format(noise))
...
### Query-Access Cifar10
    ...
    # Plot best models
    plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        "analysis/plots/cifar10/query_access/resnet110_90epochs_all_methods_sigma_12", '', 1.0,
        methods_labels_colors_dashes=[
            (all_cifar_cohen_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
            (all_exp_resnet110_fullAccess_cifar10_stability, 'Salman et al. White-box', 'orange', [6, 2]),
            (all_exp_resnet110_queryAccess_cifar10_stability, 'Salman et al. 14-Surrogates', 'g', [6, 2]),
            # (all_exp_resnet110_queryAccess_cifar10_stability_finetune_14surrogate, 'Stab+MSE 14-Surrogates', 'orange', [4, 2]),
            # (all_cifar_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
            (cifar_munet, 'Ours 1-Surrogate', 'c', [4,2]),
            # (cifar_no_denoiser_N10000, 'No denoiser', 'k', [5, 1]),
        ],
        sigma=0.12)
    plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        "analysis/plots/cifar10/query_access/resnet110_90epochs_all_methods_sigma_25", '', 1.0,
        methods_labels_colors_dashes=[
            (all_cifar_cohen_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
            (all_exp_resnet110_fullAccess_cifar10_stability, 'Salman et al. White-box', 'orange', [6, 2]),
            (all_exp_resnet110_queryAccess_cifar10_stability, 'Salman et al. 14-Surrogates', 'g', [6, 2]),
        ...
        sigma=0.25)
    plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        "analysis/plots/cifar10/query_access/resnet110_90epochs_all_methods_sigma_50", '', 1.0,
        methods_labels_colors_dashes=[
            (all_cifar_cohen_N10000, 'Cohen at al. White-box', 'b', [1, 0]),
            (all_exp_resnet110_fullAccess_cifar10_stability, 'Salman et al. White-box', 'orange', [6, 2]),
            (all_exp_resnet110_queryAccess_cifar10_stability, 'Salman et al. 14-Surrogates', 'g', [6, 2]),
            # (all_exp_resnet110_queryAccess_cifar10_stability_finetune_14surrogate, 'Stab+MSE 14-Surrogates', 'orange', [4, 2]),
            # (all_cifar_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
        ...
        sigma=0.50)
    plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        "analysis/plots/cifar10/query_access/resnet110_90epochs_all_methods_sigma_100", '', 1.0,
        methods_labels_colors_dashes=[
            (all_cifar_cohen_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
            (all_exp_resnet110_fullAccess_cifar10_stability, 'Salman et al. White-box', 'orange', [6, 2]),
            (all_exp_resnet110_queryAccess_cifar10_stability, 'Salman et al. 14-Surrogates', 'g', [6, 2]),
    ...
    plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        ...
        methods_labels_colors_dashes=[
            ...
            (all_exp_resnet110_queryAccess_cifar10_stability_finetune_14surrogate, 'Stab+MSE 14-Surrogates', 'orange', [4, 2]),
            (cifar_munet, 'Ours 1-Surrogate', 'c', [6,2]),
...
#################################################################################################################
    ...
    for arch in imagenet_archs:
        ...
        imagenet_results[arch].imagenet_ours = [
                Line(ApproximateAccuracy("certificates/our-imagenet-sigma{1:.2f}_{0}".format(arch, noise)), "$\sigma = {0:.2f}$".format(noise))
            for noise in [0.12, 0.25, 0.5, 1.0]
            ]
    ...
    for arch in imagenet_archs:
        ...
        ### Query-Access
        surrogate_models = [(imagenet_results[arch].all_imagenet_stability_obj_N10000[b], 'Salman et al. {}'.format(b), color, dashes) 
        ...
        # plot_certified_accuracy_one_sigma_best_model_multiple_methods(
        #     "analysis/plots/imagenet/query_access/MODEL_{}_stability_sigma_12_with_surrogate".format(arch), 'blackbox_imagenet_best_models', 1.0,
        #     methods_labels_colors_dashes=[
        #         (imagenet_results[arch].cohen_training_N10000, '[2] White-box', 'b', [1, 0]),] + surrogate_models + [
        #         # (imagenet_results[arch].all_imagenet_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
        #         # (imagenet_results[arch].imagenet_no_denoiser_N10000, 'No denoiser', 'k', [5, 1]),
        #         (imagenet_results[arch].imagenet_ours, 'Ours-resnet18', 'c', [4, 1]),
        #     ],
        #     sigma=0.12)
        
        plot_certified_accuracy_one_sigma_best_model_multiple_methods(
            "analysis/plots/imagenet/query_access/MODEL_{}_stability_sigma_25_with_surrogate".format(arch), '', 1.0,
            methods_labels_colors_dashes=[
                (imagenet_results[arch].cohen_training_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
                (imagenet_results[arch].all_imagenet_stability_obj_N10000[arch], 'Salman et al. White-box', 'r', [4, 2]),] + surrogate_models + [
                # (imagenet_results[arch].all_imagenet_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
                # (imagenet_results[arch].imagenet_no_denoiser_N10000, 'No denoiser', 'k', [5, 1]),
                (imagenet_results[arch].imagenet_ours, 'Ours-resnet18', 'c', [4, 1]),
            ],
            sigma=0.25)
        plot_certified_accuracy_one_sigma_best_model_multiple_methods(
            "analysis/plots/imagenet/query_access/MODEL_{}_stability_sigma_50_with_surrogate".format(arch), '', 1.0,
            methods_labels_colors_dashes=[
                (imagenet_results[arch].cohen_training_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
                (imagenet_results[arch].all_imagenet_stability_obj_N10000[arch], 'Salman et al. White-box', 'r', [4, 2]),] + surrogate_models + [
                # (imagenet_results[arch].all_imagenet_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
            ...
            sigma=0.50)
        plot_certified_accuracy_one_sigma_best_model_multiple_methods(
            "analysis/plots/imagenet/query_access/MODEL_{}_stability_sigma_100_with_surrogate".format(arch), '', 1.0,
            methods_labels_colors_dashes=[
                (imagenet_results[arch].cohen_training_N10000, 'Cohen et al. White-box', 'b', [1, 0]),
                (imagenet_results[arch].all_imagenet_stability_obj_N10000[arch], 'Salman et al. White-box', 'r', [4, 2]),] + surrogate_models + [
                # (imagenet_results[arch].all_imagenet_denoising_obj_denoisers, 'MSE', 'r', [2, 4]),
...
# Latex
    for arch in imagenet_archs:
        ...
        latex_table_certified_accuracy_upper_envelope(
            "analysis/latex/imagenet_certified_ours_{}_blackbox".format(arch), 0.25, 1.5, 0.25, 
                imagenet_results[arch].imagenet_ours)    
    ...
    latex_table_certified_accuracy_upper_envelope(
        "analysis/latex/queryAccess_cifar10_certified_our", 0.25, 1.5, 0.25, 
            cifar_munet)
